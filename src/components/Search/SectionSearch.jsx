...
const SectionSearch = ({ title, data, viewAllLink }) => {
  const { isLoading, setIsLoading } = useApi(null, null, null);
    ...
    <>
      <div className="p-4 min-[1024px]:ml-64 h-fit">
        ...
        {isLoading ? (
          ...
          <Link to={viewAllLink}>
            <div className="flex items-center flex-row relative top-[1px] hover:bg-zinc-900 transition-all hover:underline duration-300 hover:scale-105 justify-center w-24 h-10 gap-1 rounded-full text-sm ">
        ...
        <div className="overflow-x-auto no-scrollbar flex">
          <div
            className={`grid min-[1024px]:grid-rows-3 grid-flow-col-dense gap-4 min-w-max transition-all ease-in-out duration-1000 `}
          >
